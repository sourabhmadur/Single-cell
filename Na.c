...
#define dt _nt->_dt
#define G_Na _p[0]
#define tau_d_Na _p[1]
#define tau_f_Na _p[2]
#define ina _p[3]
#define d_Na _p[4]
#define f_Na _p[5]
#define ena _p[6]
#define d_Na_inf _p[7]
#define f_Na_inf _p[8]
#define Dd_Na _p[9]
#define Df_Na _p[10]
#define v _p[11]
#define _g _p[12]
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   Dd_Na = ( d_Na_inf - d_Na ) / tau_d_Na ;
   Df_Na = ( f_Na_inf - f_Na ) / tau_f_Na ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 settables ( _threadargscomma_ v ) ;
 Dd_Na = Dd_Na  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau_d_Na )) ;
 Df_Na = Df_Na  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau_f_Na )) ;
 return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   settables ( _threadargscomma_ v ) ;
    d_Na = d_Na + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_d_Na)))*(- ( ( ( d_Na_inf ) ) / tau_d_Na ) / ( ( ( ( - 1.0 ) ) ) / tau_d_Na ) - d_Na) ;
    f_Na = f_Na + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_f_Na)))*(- ( ( ( f_Na_inf ) ) / tau_f_Na ) / ( ( ( ( - 1.0 ) ) ) / tau_f_Na ) - f_Na) ;
   }
  return 0;
}
