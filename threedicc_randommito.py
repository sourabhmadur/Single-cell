...
for N_cells in ncells:
	...
	Pmito = [ [[[] for i in range(N_cells)] for i in range(N_cells)] for i in range(N_cells) ]
	for i in range(N_cells):
		for j in range(N_cells):
			for k in range(N_cells):
				Pmito[i][j][k] = (0.10871 + random.randrange(8)*0.01)
				cells[i][j][k].Pmito_conpu = Pmito[i][j][k]
	...
	np.save('t.npy', t)
	np.save('Pmito_v_ncells_3d_'+str(N_cells)+'.npy', Pmito)
