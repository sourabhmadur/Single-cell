...
def initialize_network(ncells): 
	cells = [[[] for i in range(ncells)] for i in range(ncells)]
	gap_junctions_f_x=[]
	gap_junctions_b_x=[]
	gap_junctions_f_y=[]
	gap_junctions_b_y=[]
...
#	 	cells.append(h.Section())
		 	cells[i][j] = h.Section()
		 	define_geometry(cells[i][j])
		 	insert_mechanisms(cells[i][j])
	...
	for i in range(ncells):
		...
		for j in range(ncells-1):
			...
			gj_fx.append(h.gap())
			gj_bx.append(h.gap())
			gj_fx[j].loc(cells[i][j](1))
			gj_bx[j].loc(cells[i][j+1](0))
			h.setpointer(cells[i][j+1](0)._ref_v,'vgap',gj_fx[j])  
			h.setpointer(cells[i][j](1)._ref_v,'vgap',gj_bx[j])

			gj_fy.append(h.gap())
			gj_by.append(h.gap())
			gj_fy[j].loc(cells[j][i](1))
			gj_by[j].loc(cells[j+1][i](0))
			h.setpointer(cells[j+1][i](0)._ref_v,'vgap',gj_fy[j])  
			h.setpointer(cells[j][i](1)._ref_v,'vgap',gj_by[j])
...
def  set_recording_vectors(cells):
	...
	for i in range(len(cells)):
		for j in range(len(cells)):
			v[i][j] = h.Vector()
			v[i][j].record(cells[i][j](0.5)._ref_v)
	return v,t
...
for N_cells in ncells:
	...
	for i in range(N_cells):
		for j in range(N_cells):
			p.plot(t,v_n[i][j] , label = 'cell '+ str(i)+ str(j))
