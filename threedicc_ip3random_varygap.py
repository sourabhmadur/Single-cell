...
ncells = 3
gj_res = [0.36,3.6, 30.6, 300.6, 3000.6]		#(megaohm)
for gjr in tqdm(gj_res):
	gap_junctions,cells = initialize_network(ncells)
	# gap_junctions_f_x,gap_junctions_f_y,gap_junctions_b_x,gap_junctions_b_y,cells = initialize_network(ncells)

	v,t = set_recording_vectors(cells)

	ip3 = [ [[[] for i in range(ncells)] for i in range(ncells)] for i in range(ncells) ]
	for i in range(ncells):
		for j in range(ncells):
			for k in range(ncells):
	...
	np.save('v_3cells_3d_gjres_'+str(gjr)+'.npy', v)
	np.save('t.npy', t)
	np.save('ip3_v_3cells_3d_gj_'+str(gjr)+'.npy', ip3)


	# p.figure()

	# v_n = np.load('v_3cells_3d_gjres_'+str(gjr)+'.npy')
	# t = np.load('t.npy')
	# for i in range(ncells):
	# 	for j in range(ncells):
	# 		for k in range(ncells):
	# 			p.plot(t,v_n[i][j][k] , label = 'cell '+ str(i)+ str(j)+str(k),color = (i/(ncells),j/(ncells)*1.1,k/(ncells)*1.1))
	...
	# p.legend(loc = 'upper center', ncol = 3*ncells, bbox_to_anchor=(0.5,1.1))
p.show()
